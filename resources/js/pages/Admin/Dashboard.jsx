...
const AdminDashboard = () => {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    queryKey: ["admin-stats", selectedPeriod],
    queryFn: async () => {
      return await AdminService.getAdminStats(selectedPeriod);
    },
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {["7", "30", "90"].map((period) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium ${
                  selectedPeriod === period ? "bg-primary-600 text-white" : "bg-white text-gray-700 hover:bg-gray-50"
                }`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {statsLoading ? (
            ...
            <>
              <StatCard title="Total Organizers" value={stats?.total_organizers} change={stats?.organizers_change} changeType={stats?.organizers_change > 0 ? "positive" : "negative"} icon={Users} color="primary" />
             
              <StatCard title="Total Attendees" value={stats?.total_attendees} change={stats?.organizers_change} changeType={stats?.organizers_change > 0 ? "positive" : "negative"} icon={Users} color="primary" /> <StatCard title="Pending Verification" value={stats?.pending_organizers} icon={Clock} color="warning" />
              <StatCard title="Total Events" value={stats?.total_events} change={stats?.events_change} changeType={stats?.events_change > 0 ? "positive" : "negative"} icon={Calendar} color="success" />
              <StatCard title="Platform Users" value={stats?.total_users} change={stats?.users_change} changeType={stats?.users_change > 0 ? "positive" : "negative"} icon={Shield} color="primary" />
                <StatCard title="Verified Organizers" value={stats?.verified_organizers || 0} icon={CheckCircle} color="success"/>
...
<div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
  ...
  >
    ...
    <p className="text-lg font-semibold text-blue-800 text-center">User Management</p>
    <p className="text-sm text-blue-600 text-center mt-1">View, edit, or manage platform users</p>
  </div>
  ...
  >
    ...
    <p className="text-lg font-semibold text-yellow-800 text-center">Event Management</p>
    <p className="text-sm text-yellow-600 text-center mt-1">Review, edit, or approve events</p>
  </div>
  ...
  >
    ...
    <p className="text-lg font-semibold text-green-800 text-center">Organizer Management</p>
    <p className="text-sm text-green-600 text-center mt-1">Verify or manage organizers</p>
  </div>
</div>
                ...
                <div key={activity.id} className="flex items-start space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center mt-1 space-x-2">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getActivityColor(activity.status)}`}>{activity.status}</span>
                    </div>
                  </div>
                </div>
              ))}
